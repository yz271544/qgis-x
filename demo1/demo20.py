...
def add_line(layer_name: str, lines: list[list[tuple[float, float]]], color: str,
    ...
    qt_stroke_style = QtCore.Qt.SolidLine
    # Apply stroke style
    if stroke_style == "dash":
        qt_stroke_style = QtCore.Qt.DashLine
    elif stroke_style == "dot":
        qt_stroke_style = QtCore.Qt.DotLine
    elif stroke_style == "dashdot":
        qt_stroke_style = QtCore.Qt.DashDotLine
    elif stroke_style == "dashdotdot":
        qt_stroke_style = QtCore.Qt.DashDotDotLine
    else:
        qt_stroke_style = QtCore.Qt.SolidLine
    ...
    base_symbol_layer = QgsSimpleLineSymbolLayer(QtGui.QColor(color))
    base_symbol_layer.setPenStyle(qt_stroke_style)
    base_symbol_layer.setColor(QtGui.QColor(color))
    base_symbol_layer.setWidth(width)
    # Add the line layer to the symbol
    # base_symbol.setColor(QtGui.QColor(color))
    # base_symbol.setWidth(width)
    # base_symbol.appendSymbolLayer(base_symbol_layer)
    base_symbol.changeSymbolLayer(0, base_symbol_layer)
...
def add_scale_bar(layout, map_item: QgsLayoutItemMap):
    # Create a scale bar item
    scale_bar = QgsLayoutItemScaleBar(layout)
    scale_bar.setStyle('Single Box') #Line Ticks Up
    ...
    # scale_bar.setSegmentSize(100)  # Segment size in meters
    # scale_bar.setHeight(5)  # Height of the scale bar in millimeters
    # scale_bar.setFont(QtGui.QFont('Arial', 12))
    # scale_bar.setPenWidth(0.5)
    # scale_bar.setFrameEnabled(True)
    scale_bar.setFrameStrokeWidth(QgsLayoutMeasurement(0.5, QgsUnitTypes.LayoutMillimeters))

    # Position the scale bar
    scale_bar.attemptSetSceneRect(QtCore.QRectF(10, 10, 100, 20))  # Adjust position and size as needed
...
if __name__ == '__main__':
    ...
    lineLayer = add_line("应急道路", [
        [(111.4857822, 40.726082), (111.4866153, 40.7256001), (111.4882815, 40.726957), (111.4901084, 40.7284965),
         (111.4901084, 40.7284965)]], "#e77148", 2, "dashdotdot",[8], ["#00cd52"], [0.4])
